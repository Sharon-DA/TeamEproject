...
const SlimNavItem = ({ to, icon, label, isActive }: { to: string, icon: React.ReactNode, label: string, isActive: boolean }) => {
    ...
    return (
        ...
        >
            {isActive && (
                <div className="absolute right-0 top-1/2 -translate-y-1/2 w-[3px] h-6 bg-nile-green rounded-l-full animate-in slide-in-from-right-1" />
            )}
            <div className={`
            ...
            <span className={`
                ...
                ${isActive ? 'text-black' : 'text-black opacity-40 group-hover:opacity-100'}
            `}>
                {label}
            </span>
        </div>
...
const EmployerLayout = () => {
    ...
    const menuItems = [
        ...
        { name: 'FEED', path: '/employer/feed', icon: <MessageSquare /> },
                ...
                <header className="h-14 border-b-[2px] border-black flex items-center justify-between px-6 bg-white/80 backdrop-blur-md z-20 flex-shrink-0">
                    ...
                    <div className="flex items-center space-x-6">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex items-center space-x-2.5 cursor-pointer group" onClick={() => navigate('/employer/profile')}>
                                <div className="text-right hidden sm:block">
                                    <p className="text-[9px] font-black uppercase leading-none">{userName}</p>
                                    <p className="text-[7px] font-bold text-black/40 uppercase mt-0.5 tracking-wider">{companyName}</p>
                                </div>
                                <div className="w-8 h-8 rounded-lg border-[2px] border-black shadow-[2px_2px_0px_0px_rgba(30,73,157,0.1)] group-hover:translate-x-0.5 group-hover:translate-y-0.5 group-hover:shadow-none transition-all overflow-hidden text-left">
                                     <Avatar name={userName} size="sm" />
                                </div>
                            </div>
                        </div>
