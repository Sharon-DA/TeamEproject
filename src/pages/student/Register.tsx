...
import { User, Mail, AtSign, Building, ShieldCheck, GraduationCap } from 'lucide-react';
...
const Register = () => {
    ...
    const roleConfig: Record<string, { title: string, subtitle: string, icon: React.ReactNode }> = {
        student: { title: 'NILE STUDENT', subtitle: `${typeParam.toUpperCase()} ACADEMIC TRACK`, icon: <GraduationCap size={48} strokeWidth={2.5} /> },
        employer: { title: 'CAREER PARTNER', subtitle: 'RECRUITMENT & TALENT HUB', icon: <Building size={48} strokeWidth={2.5} /> },
        staff: { title: 'NILE STAFF', subtitle: 'ADMINISTRATIVE SYSTEMS', icon: <ShieldCheck size={48} strokeWidth={2.5} /> }
    };
    ...
    const leftPanelContent = (
        <div className="flex flex-col items-center text-center">
            ...
            <div className="space-y-1">
                ...
                <p className="text-[9px] font-black text-white/50 uppercase tracking-[0.4em]">{config.subtitle}</p>
            </div>
        </div>
    );
        ...
        <AuthLayout leftContent={leftPanelContent}>
            <div className="space-y-10 anime-fade-in text-left">
                <div className="space-y-2">
                    <h1 className="text-4xl font-black text-black uppercase tracking-tight">Join Hub</h1>
                    <p className="text-[9px] font-black text-nile-blue uppercase tracking-[0.25em]">
                        CREATE YOUR {roleParam} IDENTITY
                    </p>
                </div>
                ...
                <div className="pt-6 border-t-[2px] border-black/5 text-center">
                    <p className="text-[9px] font-black text-black/30 uppercase tracking-[0.15em] mb-4">ALREADY ON THE HUB?</p>
                    <button 
